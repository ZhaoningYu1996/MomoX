...
def sanitize(mol, add_H):
    try:
        smiles = get_smiles(mol)
        mol = get_mol(smiles, add_H)
...
def get_clique_mol(mol, atoms):
    ...
    new_mol = sanitize(new_mol, False) #We assume this is not None
    return new_mol
...
def tensorize(mol_tree, assm=True):
    mol_tree.recover()
    if assm:
        mol_tree.assemble()
...
def enum_assemble(node, neighbors, prev_nodes=[], prev_amap=[]):
    ...
    def search(cur_amap, depth):
        ...
        for amap in cand_amap:
            ...
            cand_mol = sanitize(cand_mol, False)
            if cand_mol is None:
                continue
