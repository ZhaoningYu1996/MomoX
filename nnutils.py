...
def create_var(tensor, requires_grad=None, device="cpu"):
...
def avg_pool(all_vecs, scope, dim, device):
    size = create_var(torch.Tensor([le for _,le in scope]), device=device)
