...
class JTNNDecoder(nn.Module):
    ...
    def __init__(self, vocab, hidden_size, latent_size, embedding, mask, device):
    ...
    def forward(self, mol_batch, x_tree_vecs):
        ...
        pred_hiddens.append(create_var(torch.zeros(len(mol_batch),self.hidden_size), device=self.device))
        pred_targets.extend([mol_tree.nodes[0].wid for mol_tree in mol_batch])
        pred_contexts.append( create_var( torch.LongTensor(range(batch_size)), device=self.device) )

        max_iter = max([len(tr) for tr in traces])
        padding = create_var(torch.zeros(self.hidden_size), False, device=self.device)
        h = {}
        ...
        stop_hidden = torch.cat([cur_x,cur_o], dim=1)
        stop_hiddens.append( stop_hidden )
        stop_contexts.append( create_var( torch.LongTensor(range(batch_size)), device=self.device) )
